...
import { Student, CeremonyStatus, ceremonyStatusToLabel, DormitoryCeremonyAbsence } from '../../types';
...
interface TodayRecordsTableProps {
  records: DormitoryCeremonyAbsence[];
...
const TodayRecordsTable: React.FC<TodayRecordsTableProps> = ({ records, onStudentSelect }) => {
    ...
    return (
        <div className="p-2 sm:p-6 bg-slate-50 border border-slate-200 rounded-lg mb-8">
            <h3 className="text-2xl font-bold text-slate-800 mb-4 text-center">بيانات غياب المراسم اليوم</h3>
                        ...
                        {records.map((record) => {
                            ...
                            return (
                                <tr key={record.id} className="border-b hover:bg-slate-50">
                                    ...
                                    <td className="px-2 py-3 sm:px-6 sm:py-4 whitespace-nowrap"><span className={`px-2 py-1 text-xs font-semibold rounded-full ${statusInfo.bg} ${statusInfo.text}`}>{statusLabel}</span></td>
                                </tr>
                            );
                        })}
                    </tbody>
...
interface CeremonyAbsenceViewProps {
  onStudentSelect: (studentId: string) => void;
}
...
export const CeremonyAbsenceView: React.FC<CeremonyAbsenceViewProps> = ({ onStudentSelect }) => {
    ...
    const todayRecords = useMemo(() => {
        ...
        return ceremonyAbsences
            .filter(r => r.date === today)
            .sort((a, b) => a.studentName.localeCompare(b.studentName));
    }, [ceremonyAbsences]);
    
    useEffect(() => { if (selectedDormitoryId) setInteractionStarted(true); }, [selectedDormitoryId]);
    ...
    const toggleStatus = (studentId: string, status: CeremonyStatus) => {
        setStudentStatuses(prev => {
            ...
            if (newMap.get(studentId) === status) newMap.delete(studentId);
            else newMap.set(studentId, status);
            return newMap;
        });
    };
                        ...
                        {filteredStudents.length > 0 ? (
                            <div>
                                ...
                                <div className="overflow-x-auto bg-white rounded-lg border">
                                    <table className="w-full text-sm text-right text-slate-600">
                                        <thead className="text-xs text-slate-700 uppercase bg-slate-100"><tr><th className="px-2 py-3 sm:px-6 whitespace-nowrap">اسم الطالب</th><th className="px-2 py-3 sm:px-6 text-left whitespace-nowrap">حالة الحضور</th></tr></thead>
